...
proc putU16LE*(buf: var openArray[byte], idx: var int, v: uint16) {.inline.} =
  buf[idx] = byte(v and 0xFF)
  inc idx
  buf[idx] = byte((v shr 8) and 0xFF)
  inc idx
...
proc putU32LE*(buf: var openArray[byte], idx: var int, v: uint32) {.inline.} =
  ...
  buf[idx] = byte((v shr 8) and 0xFF)
  inc idx
  buf[idx] = byte((v shr 16) and 0xFF)
  inc idx
  buf[idx] = byte((v shr 24) and 0xFF)
  inc idx
...
proc takeU16LE*(data: openArray[byte], idx: var int): uint16 {.inline.} =
  let a = uint16(data[idx])
  inc idx
  let b = uint16(data[idx])
  inc idx
  result = a or (b shl 8)
...
proc takeU32LE*(data: openArray[byte], idx: var int): uint32 {.inline.} =
  ...
  inc idx
  let b1 = uint32(data[idx])
  inc idx
  let b2 = uint32(data[idx])
  inc idx
  let b3 = uint32(data[idx])
  inc idx
