...
type
  ...
  NativeExportConfig* = object
    ...
    ## Raise an error when an exclusion matches no public procedure.
    requireMatches*: bool
...
proc validateNativeExportConfig*(config: NativeExportConfig) =
  ...
  for selector in config.excludeProcs:
    if selector.name.len == 0:
      fail("native export exclusion has an empty procedure name")
    if '`' in selector.name:
      fail("native export exclusion names must omit backticks: " & selector.name)
    if selector.source.isAbsolute:
      fail("native export exclusion source must be relative: " & selector.source)
...
func matches*(selector: NativeProcSelector; source, name: string): bool =
  ...
  let sourceMatches = selector.source.len == 0 or
...
proc parseSelector(node: JsonNode; index: int): NativeProcSelector =
  let description = "excludeProcs[" & $index & "]"
  ...
  result = excludeProc(
    node["name"].getStr,
    if node.hasKey("source"): node["source"].getStr else: "",
  )

proc loadNativeExportConfig*(path: string): NativeExportConfig =
  ## Loads ``excludeProcs`` and ``requireMatches`` from a JSON file.
  let root =
    try:
      parseFile(path)
  ...
  root.rejectUnknownFields(
    ["excludeProcs", "requireMatches"], "native export config")
  ...
  if root.hasKey("excludeProcs"):
    ...
    for item in items.items:
      selectors.add parseSelector(item, selectors.len)
  ...
  result = initNativeExportConfig(selectors, requireMatches)
