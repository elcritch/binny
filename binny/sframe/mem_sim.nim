...
proc loadU64*(mem: SimMemory, address: uint64): uint64 =
  ...
  when system.cpuEndian == littleEndian:
    result = (
      uint64(mem.bytes[i + 0]) or (uint64(mem.bytes[i + 1]) shl 8) or
      (uint64(mem.bytes[i + 2]) shl 16) or (uint64(mem.bytes[i + 3]) shl 24) or
      (uint64(mem.bytes[i + 4]) shl 32) or (uint64(mem.bytes[i + 5]) shl 40) or
      (uint64(mem.bytes[i + 6]) shl 48) or (uint64(mem.bytes[i + 7]) shl 56)
    )
  else:
    result = (
      (uint64(mem.bytes[i + 0]) shl 56) or (uint64(mem.bytes[i + 1]) shl 48) or
      (uint64(mem.bytes[i + 2]) shl 40) or (uint64(mem.bytes[i + 3]) shl 32) or
      (uint64(mem.bytes[i + 4]) shl 24) or (uint64(mem.bytes[i + 5]) shl 16) or
      (uint64(mem.bytes[i + 6]) shl 8) or (uint64(mem.bytes[i + 7]))
    )
